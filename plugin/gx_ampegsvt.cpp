...
// Custom deleter for PluginLV2 smart pointers
struct PluginLV2Deleter {
    void operator()(PluginLV2* p) const {
        if (p) {
            if (p->activate_plugin) {
                p->activate_plugin(false, p);
            }
...
using unique_plug = std::unique_ptr<PluginLV2, PluginLV2Deleter>;
...
private:
  ...
  unique_plug      ampegsvtp1;
  std::vector<unique_plug> ampegsvtone;
  unique_plug      ampegsvtp3;
  std::vector<unique_plug> ampegsvmid;
  unique_plug      ampegsvtp5;
  unique_plug      valve;
  unique_plug      cabsim;
...
Gx_ampegsvt_::Gx_ampegsvt_() :
  ...
  ampegsvtone(TONE_COUNT),
  ampegsvtp3(ampegsvtp3::plugin()),
  ampegsvmid(MID_COUNT),
...
Gx_ampegsvt_::~Gx_ampegsvt_()
{
  // All memory is managed by unique_ptr now
};
...
void Gx_ampegsvt_::init_dsp_(uint32_t rate)
{
  ...
  ampegsvtp1->set_samplerate(fSamplingFreq, ampegsvtp1.get()); // init the DSP class
  ampegsvtp3->set_samplerate(fSamplingFreq, ampegsvtp3.get()); // init the DSP class
  ampegsvtp5->set_samplerate(fSamplingFreq, ampegsvtp5.get()); // init the DSP class
  valve->set_samplerate(fSamplingFreq, valve.get()); // init the DSP class
  cabsim->set_samplerate(fSamplingFreq, cabsim.get()); // init the DSP class
  for(uint32_t i=0; i<TONE_COUNT; i++) {
    ampegsvtone[i] = unique_plug(tone_model[i]());
    ampegsvtone[i]->set_samplerate(fSamplingFreq, ampegsvtone[i].get());  
  }
  for(uint32_t i=0; i<MID_COUNT; i++) {
    ampegsvmid[i] = unique_plug(mid_model[i]());
    ampegsvmid[i]->set_samplerate(fSamplingFreq, ampegsvmid[i].get());  
  }
}
...
void Gx_ampegsvt_::connect_(uint32_t port,void* data)
{
  switch ((PortIndex)port)
    ...
    case BYPASS: 
      ...
      break;
    case DRIVE: 
      drive = static_cast<float*>(data);
      break;
    case MIX: 
      mix = static_cast<float*>(data);
      break;
    case LOWSWITCH: 
...
void Gx_ampegsvt_::activate_f()
{
  ...
  if (ampegsvtp1->activate_plugin !=0)
    ampegsvtp1->activate_plugin(true, ampegsvtp1.get());
  if (ampegsvtp3->activate_plugin !=0)
    ampegsvtp3->activate_plugin(true, ampegsvtp3.get());
  if (ampegsvtp5->activate_plugin !=0)
    ampegsvtp5->activate_plugin(true, ampegsvtp5.get());
  if (valve->activate_plugin !=0)
    valve->activate_plugin(true, valve.get());
  if (cabsim->activate_plugin !=0)
    cabsim->activate_plugin(true, cabsim.get());
  for(uint32_t i=0; i<TONE_COUNT; i++) {
    if (ampegsvtone[i]->activate_plugin !=0)
      ampegsvtone[i]->activate_plugin(true, ampegsvtone[i].get());  
  }
  for(uint32_t i=0; i<MID_COUNT; i++) {
    if (ampegsvmid[i]->activate_plugin !=0)
      ampegsvmid[i]->activate_plugin(true, ampegsvmid[i].get());  
  }
}
...
void Gx_ampegsvt_::run_dsp_(uint32_t n_samples)
{
  ...
  if (needs_ramp_down) {
    ...
    if (ramp_down <= 0.0) {
      ...
      ampegsvtp1->clear_state(ampegsvtp1.get());
      ampegsvtone[model]->clear_state(ampegsvtone[model].get());
      ampegsvtp3->clear_state(ampegsvtp3.get());
      ampegsvmid[midmodel]->clear_state(ampegsvmid[midmodel].get());
      ampegsvtp5->clear_state(ampegsvtp5.get());
      cabsim->clear_state(cabsim.get());
      needs_ramp_down = false;
  ...
  if (!bypassed) {
	  ampegsvtp1->mono_audio(static_cast<int>(ReCount), buf, buf, ampegsvtp1.get());
	  ampegsvtone[model]->mono_audio(static_cast<int>(ReCount), buf, buf, ampegsvtone[model].get());
	  ampegsvtp3->mono_audio(static_cast<int>(ReCount), buf, buf, ampegsvtp3.get());
	  ampegsvmid[midmodel]->mono_audio(static_cast<int>(ReCount), buf, buf, ampegsvmid[midmodel].get());
	  ampegsvtp5->mono_audio(static_cast<int>(ReCount), buf, buf, ampegsvtp5.get());
	  valve->mono_audio(static_cast<int>(ReCount), buf, buf, valve.get());
	  if (cabmodel)cabsim->mono_audio(static_cast<int>(ReCount), buf, buf, cabsim.get());
  }
	if (fact>1) {
    ...
    for (uint32_t i=0; i<n_samples; i++) {
        output[i] = clean_buf[i] * (1.0 - mix_val) + output[i] * mix_val;
    }
}
...
void Gx_ampegsvt_::connect_all__ports(uint32_t port, void* data)
{
  ...
  ampegsvtp1->connect_ports(port,  data, ampegsvtp1.get());
  ampegsvtp3->connect_ports(port,  data, ampegsvtp3.get());
  ampegsvtp5->connect_ports(port,  data, ampegsvtp5.get());
  valve->connect_ports(port,  data, valve.get());
  cabsim->connect_ports(port,  data, cabsim.get());
  for(uint32_t i=0; i<TONE_COUNT; i++) {
    ampegsvtone[i]->connect_ports(port,  data, ampegsvtone[i].get());  
  }
  for(uint32_t i=0; i<MID_COUNT; i++) {
    ampegsvmid[i]->connect_ports(port,  data, ampegsvmid[i].get());  
  }
}
